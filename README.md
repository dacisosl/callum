...
- **보드**: 칼럼마다 제목 막대 아래에 `+` 버튼이 있고, 새 카드는 맨 위에 추가됩니다. 오른쪽 아래 카드 추가 버튼은 첫 칼럼에 추가합니다. 왼쪽 위 로고나 보드 이름 메뉴의 "모든 보드 보기"로 홈에 돌아갑니다.
- **공유 보기**: `?share=토큰` 주소로 열리며 로그인 없이 읽기 전용으로 표시됩니다.
